...
def create_2d_dataset(nii_images_path, nii_masks_path, test_data_percentage=0.3, val_data_percentage=0.15, scaler=None, clip = False):
    ...
    train_images, train_masks, test_images, test_masks, val_images, val_masks = image_tensors, mask_tensors, [], [], [], [] # empty initialization, nothing more

    print(f"len of train images and masks are {len(train_images)} and {len(train_masks)} respectively")

    if(test_data_percentage > 0):
        train_images, test_images,  train_masks, test_masks = train_test_split(train_images,   train_masks,   test_size=test_data_percentage)
    
    if(val_data_percentage > 0):
        train_images, val_images,   train_masks, val_masks  = train_test_split(train_images,    train_masks,    test_size=val_data_percentage )
